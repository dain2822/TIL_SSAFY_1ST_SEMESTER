...
- 컴퓨터에서 문자를 표현하는 방법에 대해 이해함
- 컴퓨터에서 문자열을 저장하는 방식을 이해하고, 이를 활용할 수 있음
...
![image.png](attachment:a92666e3-3a8c-4cfe-ab5f-1fbf0dd87465:image.png)
...
![image.png](attachment:cbe39e61-4a71-43f4-b21e-d0b17cb8e88f:image.png)
...
- 컴퓨터 생산자와 소프트웨어 개발자가 여러 가지 다양한 문자에 할당할 수 있도록 하고, 이렇게 할당된 확장 부호는 표준 아스키와 같이 서로 다른 프로그램이나 컴퓨터 사이에 교환되지 못함
- 표준 아스키는 마이크로 컴퓨터 하드웨어 및 소프트웨어 사이에서 세계적으로 통용되는데 비해, 확장 아스키는 프로그램이나 컴퓨터 또는 프린터가 그것을 해독할 수 있도록 설계되어 있어야만 올바르게 해독될 수 있음

![image.png](attachment:01decedd-c52b-47c2-99ad-e6650c1aada0:image.png)

**유니코드**

- 컴퓨터가 발전하면서 미국 뿐 아니라 각 나라에서 컴퓨터가 발전함
- 각 국가들은 자국의 문자를 표현하기 위해 코드체계를 만들어서 사용하게 됨
    - 우리나라도 한글 코드 체계를 만들어 사용했고 조합형, 완성형 두 종류를 가지고 있었음
...
- 다국어 처리를  위해 표준인 *유니코드*를 만듦
    - 비영리 단체 유니코드 컨소시엄(https://home.Unicode.org)에서 관리
    - 이모지도 유니코드 문자임
    
    ![image.png](attachment:e7661a2f-5987-4bb4-947d-d9c9feb96759:image.png)
...
- 바이트 단위 저장 순서가 정해지지 않은 경우 잘못된 해석 가능성
    ...
    ![image.png](attachment:7a5141a2-3793-45cd-ac79-045354c9f8c9:image.png)
    
    *little-endian과 big-endian : 여러 바이트로 표현된 수를 바이트 단위로 저장하는 순서를 나타냄
...
- UTF-16 (in windows, java)
    - MIN : 16-bit, MAX : 32-bit(2 Bytes * 2)
- UTF-32 (in unix)
    - MIN : 32-bit, MAX : 32-bit(4 Bytes * 4)
...
⇒ 코드 체계의 핵심 : 전 세계의 모든 문자를 컴퓨터가 일관되게 표현하고 처리하는 것
...
![image.png](attachment:b7fd25a1-ac51-4389-9e75-9940e1fd4f45:image.png)
...
- 길이 외에 다른 정보도 저장
    ...
    - length : 문자열의 길이
    - hash : 문자열의 해시값으로, 딕셔너리 키로 쓸 때 사용됨
    - interned : 같은 문자열을 관리하는 플래그
    - kind : 문자열 인코딩의 크기
    - data : 문자열이 저장된 실제 메모리 주소를 가리키는 포인터

![그림 변경 필요!!!](attachment:e05991c1-f9a6-4137-9c68-1f20a8beac08:image.png)
...
- 문자 배열에 문자열을 저장할 때는 항상 마지막에 끝을 표시하는 널 문자(’\0’) 필요
...
- 텍스트 데이터의 취급 방법이 통일되어 있음
    - python 2와 달리 바이트 문자열과 Unicode 구분이 없음
    - 유니코드(Unicode) 기반이라 어떤 언어나 기호도 동일한 방식으로 처리함
- 문자열 기호
...
- Python 3은 유니코드(UTF-8)로 저장

![image.png](attachment:5bef010f-7d6e-46bd-abad-2468ab3500e2:image.png)
...
![image.png](attachment:b1c96420-cc70-414c-9bee-adc691a5abb4:image.png)

![image.png](attachment:69eab530-d22d-4279-b835-4e2e538743b5:image.png)